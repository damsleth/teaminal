...
const SHOTS_DIR = path.resolve(
  process.cwd(),
  ".tui-loop/shots/menu-and-composer",
);
...
// ---------------------------------------------------------------------------
test("activity-panel: Ctrl-A opens the Activity overlay", async ({
  terminal,
}) => {
...
// ---------------------------------------------------------------------------
test("composer-active: Enter opens a chat and Tab focuses the composer", async ({
  terminal,
}) => {
  ...
  terminal.submit();
  await expect(
    terminal.getByText("The launch notes are ready for review."),
  ).toBeVisible();

  // Tab moves focus from the list into the composer (App.tsx tab handler).
  terminal.write("\t");

  // When the composer is active its first line is prefixed with "> ".
