...
test("T1.0 sub-gate: xterm.js buffer cell API reachability", async ({ terminal }) => {
  ...
  lines.push(`  terminal.buffer: ${publicBuffer === undefined ? "undefined (not exposed)" : typeof publicBuffer}`);

  // Path 2: private _term field — TypeScript private but accessible at JS runtime
  const xtermInstance = (terminal as any)._term;
  lines.push(`  (terminal as any)._term: ${xtermInstance === undefined ? "undefined" : typeof xtermInstance}`);
  lines.push(`  _term.buffer: ${xtermInstance?.buffer === undefined ? "undefined" : typeof xtermInstance?.buffer}`);
  lines.push(`  _term.buffer.active: ${xtermInstance?.buffer?.active === undefined ? "undefined" : typeof xtermInstance?.buffer?.active}`);
  ...
  lines.push(`  getLine(0) keys: ${line0 ? JSON.stringify(Object.keys(Object.getPrototypeOf(line0) ?? {})) : "n/a"}`);
  ...
  if (cell0 !== undefined && cell0 !== null) {
    try { cellMethods.getChars = cell0.getChars?.(); } catch (e) { cellMethods.getChars = `ERROR: ${e}`; }
    try { cellMethods.getFgColor = cell0.getFgColor?.(); } catch (e) { cellMethods.getFgColor = `ERROR: ${e}`; }
    try { cellMethods.getFgColorMode = cell0.getFgColorMode?.(); } catch (e) { cellMethods.getFgColorMode = `ERROR: ${e}`; }
    try { cellMethods.getBgColor = cell0.getBgColor?.(); } catch (e) { cellMethods.getBgColor = `ERROR: ${e}`; }
    try { cellMethods.getBgColorMode = cell0.getBgColorMode?.(); } catch (e) { cellMethods.getBgColorMode = `ERROR: ${e}`; }
    try { cellMethods.isBold = cell0.isBold?.(); } catch (e) { cellMethods.isBold = `ERROR: ${e}`; }
    try { cellMethods.isInverse = cell0.isInverse?.(); } catch (e) { cellMethods.isInverse = `ERROR: ${e}`; }
      ...
      for (let col = 0; col < 100; col++) {
        ...
        if (fgMode !== undefined && fgMode > 0) {
          ...
          coloredCellSample = {
            row, col,
            char: cell.getChars?.() || " ",
            fg, fgMode, bg, bgMode, bold, inverse,
          };
          break;
        }
      }
  ...
  const gateReport = {
    ...
    accessPath: xtermInstance !== undefined
      ? "(terminal as any)._term.buffer.active.getLine(row).getCell(col)"
      : "NONE — _term not accessible",
    fallback: publicBuffer === undefined && xtermInstance === undefined
